...
var undoTodo = &cobra.Command{
	Use: "undo <index1> <index2> ...",
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) < 1 {
			fmt.Printf("Error: undo need more than 1 argument\n")
		}
		...
		for _, arg := range args {
			...
			if !items[number - 1].Completed {
				fmt.Printf("Task \"%s\" is not completed yet\n", items[number - 1].Description)
				return
			}
			items[number - 1].Completed = false
		}
